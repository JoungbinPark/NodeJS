...
// Node.js에 포함된 함수, 기능과 문법을 이용해서 웹호스팅을 할 수 있는 서버를 구축합니다. 
// Node가 제공하는 라이브러리 중 서버 구축에 필요한 기능과 함수를 담고 있는 http 모듈(객체)을 require합니다.
const http = require('http');
// http 모듈의 내용을 불러와서 http라는 변수에 저장하여 사용합니다. 
// 객체 형식으로 불러와서 저장한 형태이므로 http.함수이름(), http.변수이름 처럼 사용합니다. 

http.createServer((request, response)=>{ });
// createServer 함수 : Node.js 자바스크립트로 만든 http 서버가 설정 & 실행되게 하는 함수입니다. 
// (request, response) => { } : 서버에 클라이언트가 요청이 있을 때 실행할 명령들이 들어갑니다. 
...
//서버 변수를 사용하지 않은 간단 표현 예 : http.createServer((req, res)=>{}).list();
